...
excerpt: "Architecture essentials every Test Developer should know"
comments: true
related: true
header:
    overlay_filter: 0.25
...
## (Page) Factory
The next pattern is Page Factory or simply Factory, since it can be used without any pages.
```java
...
{% include figure image_path="/assets/images/arrow.png"%}
```java
HomePage homePage = loginAs(user);
...
```
According to [Wikipedia](https://en.wikipedia.org/wiki/Factory_method_pattern) Factory is pattern which helps to solve the problem of creating objects without specifying the exact type for each of them. In other words, it can be used whenever we create multiple objects with some additional actions done to them before using them. In the example above we have login action which obviously would be done over and over again in our test scripts. So it would be easier to implement factory method `loginAs`, which returns the `HomePage` instance with already logged in user.
